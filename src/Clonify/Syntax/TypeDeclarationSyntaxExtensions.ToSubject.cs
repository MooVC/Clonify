...
internal static partial class TypeDeclarationSyntaxExtensions
{
    ...
    ///
    /// The semantics will only be mapped if the <paramref name="syntax"/> is annotated with the Clonify attribute and it,
    ...
    /// </param>
    /// <returns>
    /// When the <paramref name="syntax"/> is annotated with the Clonify attribute and it, and its parents are marked as partial,
    ...
    public static Subject? ToSubject(this TypeDeclarationSyntax? syntax, Compilation compilation, CancellationToken cancellationToken)
    {
        ...
        if (symbol is not INamedTypeSymbol type || !type.HasClonify())
        {
            return default;
        }
