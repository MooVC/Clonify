...
using Xunit.Sdk;
using static Clonify.Snippets.Extensions;
    ...
    : DataAttribute
{
    ...
    private static Type[] FindDeclarations()
    {
        return Assembly
            ...
            .GetTypes()
            .Where(type => type.Namespace is not null && type.Namespace.StartsWith("Clonify.Snippets.Declarations", StringComparison.InvariantCulture))
            .ToArray();
    }
