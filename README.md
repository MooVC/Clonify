# Clonify [![NuGet](https://img.shields.io/nuget/v/Clonify?logo=nuget)](https://www.nuget.org/packages/Clonify/) [![GitHub](https://img.shields.io/github/license/MooVC/Clonify)](LICENSE.md)

Clonify, a .NET Roslyn source generator, automates the creation of strongly-typed wrappers around single values, improving semantic clarity, type safety, and maintainability.
...
To install Clonify, use the following command in your package manager console:

```shell
install-package Clonify
```

## Usage

Clonify turns a `class`, `record`, or `struct` into a strongly typed wrapper around a single value. To opt in, annotate a `partial` type with the `Clonify` attribute and specify the encapsulated value type.
...
[Clonify(Type = typeof(int))]
public partial struct Age;
```

### Generated Members

When applied, Clonify generates the boilerplate needed for a lightweight value object:
...
Clonify includes several analyzers to assist engineers with its usage. These are:

Rule ID                          | Category | Severity | Notes
:--------------------------------|:---------|:---------|:-------------------------------------------------------------------------
[MONFY01](docs/rules/MONFY01.md) | Usage    | Warning  | Type is not compatible with Clonify
